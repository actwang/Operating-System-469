...
void main (int argc, char *argv[])
{
  ...
  */
  ...
  num_react2 = num_CO / 4;
  num_react3 = num_SO4;
  num_procs = num_react1 + num_react2 + num_react3;
  num_S2_left = num_S2 - num_react1;
  num_CO_left = num_CO - num_react2;

  // creat mailbox handle for use
  MboxModuleInit();     //only run at boot time?
  ...
  // ditoa(num_S2, num_S2_str);  We're only injecting one each time so it makes no sense to pass it
  ...
  for (i = 0; i < num_S2; i++){
    process_create(FILE_S2, S2_handle, s_procs_completed_str, NULL);
  }
  for (i = 0; i < num_CO; i++){
