...
//	INVALID_LOCK (see synch.h). //WHY NO INVALID_LOCK IN CODE BELOW??
//-----------------------------------------------------------------------
lock_t LockCreate() {
  lock_t l;
  uint32 intrval;
...
int CondBroadcast (Cond *cond) {
  ...
  intrs = DisableIntrs ();
  dbprintf ('s', "CondBroadcast: Process %d Broadcasting on cond  %d.\n", GetCurrentPid(), (int)(cond-conds));

  while (!AQueueEmpty(&cond->waiting)) { // while there still is a process to wake up
    l = AQueueFirst(&cond->waiting);
    pcb = (PCB *)AQueueObject(l);
    if (AQueueRemove(&l) != QUEUE_SUCCESS) {    // Remove link from the queue
      printf("FATAL ERROR: could not remove link from Cond. var. queue in CondBroadcast!\n");
      exitsim();
    }
    dbprintf ('s', "CondBroadcast: Waking up PID %d.\n", (int)(GetPidFromAddress(pcb)));
    //ProcessWakeup (pcb);

    // Move unblocked processd from condvar queue into back of lock await queue
    // if ((l = AQueueAllocLink((void*) pcb) == NULL)) {
    //   printf("FATAL ERROR: could not allocate free link for lock queue in CondBroadcast!\n");
    //   exitsim();
    // }
    if (AQueueInsertLast(&(locks[cond->lock].waiting), l) != QUEUE_SUCCESS){
      printf("FATAL ERROR: could not insert new link into lock queue in CondBroadcast! \n");
      exitsim();
    }
  }
